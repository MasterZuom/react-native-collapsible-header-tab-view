...
const CollapsibleTabView = forwardRef<CollapsibleTabViewRef, CollapsibleTabViewProps>(
  (
    {
      children,
      renderHeader,
    ...
    const tryCommitLayout = useMemoizedFn(() => {
    ...
    const tabRefs = useRef(new Map<number, FlatList<any> | ScrollView | null>())
    ...
    const registerRef = useCallback((index: number, ref: FlatList<any> | ScrollView | null) => {
      if (ref) {
    ...
    const scrollTabTo = useMemoizedFn((index: number, offset: number) => {
    ...
    const syncTabOnSwitch = useMemoizedFn((newIndex: number) => {
    ...
    const handlePageSelected = useMemoizedFn((e: { nativeEvent: { position: number } }) => {
    ...
    return (
      <CollapsibleContext.Provider value={contextValue}>
        <View style={[styles.container, style]}>
            ...
            >
              {pages.map((page: React.ReactElement, i) => (
                <View key={page?.key ?? i} style={styles.page}>
                  <TabIndexContext.Provider value={i}>{page}</TabIndexContext.Provider>
                </View>
              ))}
            </PagerView>
          </Animated.View>
          ...
          {stickyEnabled && (
            <Animated.View
              style={[styles.overlay, { transform: [{ translateY: headerTranslateY }] }]}
